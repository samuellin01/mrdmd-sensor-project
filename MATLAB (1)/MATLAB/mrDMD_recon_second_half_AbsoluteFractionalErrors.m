...
clear; close all; clc
datpath = '../DATA/';
%figpath = '../FIGURES/250sensors_AE_FE_Errors2012WINTER';
figpath = '../FIGURES/FIRES_RECONS/mrDMD/2012/mdDMDsensors_2012SUMMER_';
...
%indt = linspace(943,943+31,31); %August
%indt = linspace(912,943+31,63) %July-August 2002
%indt = linspace(2483,2483+122,123) %July-Oct 2012
%indt = linspace(3092,3092+50,51) %Jan 2012 winter
...
%    %[AE, FE] = ae_fe_map(Xrecon(:,i),dat(:,:,indt(i)),mask);
...
%        saveas(gcf,png_name);
%        png_name = strcat(figpath, 'FIG_RECON_AE_FE_',num2str([month(t),day(t),year(t)],'%d_%d_%d'),'.png');
%        saveas(gcf,png_name);
...
for i=1:size(Xrecon,2)
    ...
    title(num2str([month(t),day(t),year(t)],'%d/%d/%d'));
    if PRINT_FIG
        png_name = strcat(figpath, 'FIG_RECON_AE_',num2str([month(t),day(t),year(t)],'%d_%d_%d'),'.png');
        saveas(gcf,png_name);
    end
    figure;
    ...
    title(num2str([month(t),day(t),year(t)],'%d/%d/%d'));
    if PRINT_FIG
        png_name = strcat(figpath, 'FIG_RECON_FE_',num2str([month(t),day(t),year(t)],'%d_%d_%d'),'.png');
        saveas(gcf,png_name);
    end
end
