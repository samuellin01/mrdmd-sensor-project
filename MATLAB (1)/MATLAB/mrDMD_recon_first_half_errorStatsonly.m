...
% Read in the data
datX = ncread([datpath,'Downsampled_Annual_2000to2016_PM25_nocompression.nc'], 'Band1'); 
mask = ncread([datpath,'PM25_mask_array_coarse2_final.nc'],'Band1');

% First 4096 days of data, excluding corrupted dates
dat = datX(:,:,setdiff((1:4097),[3291]));
dat2 = datX(:,:,setdiff((2082:6180),[3291,5689,5690]));
...
rng(10)
indt = 2014 + randperm(2082, 400) %interpolate
...
[~,~,piv] = qr(Lib.','vector');
%sens = piv(1:size(Lib,2));     %list num of sensors    mkelp
sens = piv(1:10);     %list num of sensors    mkelp
...
%mkelp
time = setdiff((2082:6180),[3291, 5689, 5690]); %mkelp
Y = zeros(length(mask(mask==1)),size(dat2,3));
for i=1:size(dat,3)
    Band = dat2(:,:,i);
    Y(:,i) = Band(mask==1);
end
...
for i=1:size(Xrecon,2)
    ...
    [rmse, mpe] = rmse_mpe(Xrecon(:,i),dat2(:,:,indt(i)),mask);
