...
%(sub = 20, so Xaug contains every 20th column of Xraw)
if (stackflag) % ???
    Xaug = [Xaug(:, 1:end-1); Xaug(:, 2:end)];
    % X and X' stacked on top of one another, but why?
end
X = Xaug(:, 1:end-1); % data matrix X
Xp = Xaug(:, 2:end); % data matrix X'
...
if (r==0) % optimize projection rank at each level
    ...
    r = min(size(U,2),r); % use either use-specified rank or # of columns of U
end
...
Atilde = U_r' * Xp * V_r / S_r; %the rxr projection of A onto the POD modes

[W, D] = eig(Atilde); %eigendecomposition of Atilde
lambda = diag(D); %the eigenvalues of Atilde (and A)

Phi = Xp * V_r / S_r * (W/D); %the columns of Phi are the DMD modes


%% compute power of modes
Vand = zeros(r, size(X, 2)); % Vandermonde matrix: powers of the r eigenvalues
for k = 1:size(X, 2),
    Vand(:, k) = lambda.^(k-1);
end;

% the next 5 lines follow Jovanovic et al, 2014 code:
% file:///C:/Users/15104/Downloads/jovanovic-etal2014.pdf
% something about minimizing the least-squares deviation between the matrix
% of snapshots X and the linear combination of the dmd modes
% minimize J(a) = ||G - W*diag(b)*Vand||_Atilde^2
G = S_r * V_r';
...
%observations: Every recursion, T halves, rho doubles
thislevel.T = T;
thislevel.rho = rho;
